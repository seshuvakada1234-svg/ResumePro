...
export const ResumeForm: React.FC<ResumeFormProps> = ({ initialData, onChange, onSave, isSaving }) => {
  ...
  const inputClass =
    'w-full px-3 py-2 md:py-2.5 min-h-[44px] md:min-h-0 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition-all';
  ...
  return (
    <form onSubmit={handleSubmit(onSave)} className="space-y-8 p-6 bg-white rounded-xl shadow-sm border border-gray-100">
      ...
      <section className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-indigo-50 rounded-xl border border-indigo-100">
          <div className="flex items-center gap-3">
            <div className="bg-indigo-600 p-2 rounded-lg text-white">
              <Layout size={20} />
            </div>
            <div>
              <p className="text-sm font-bold text-indigo-900">Current Template</p>
              <p className="text-xs text-indigo-600 capitalize font-medium">
                Selected: {watchedData.template.replace('-', ' ')}
              </p>
            </div>
          </div>
          <button
            ...
            className="flex items-center justify-center gap-2 px-6 py-2.5 bg-white text-indigo-600 border-2 border-indigo-600 rounded-xl font-bold hover:bg-indigo-600 hover:text-white transition-all shadow-sm"
          >
            Change Template
          </button>
        </div>
              ...
              <div className="relative w-32 h-32">
                <div className="w-full h-full rounded-full overflow-hidden border-4 border-white shadow-xl transition-all duration-500 group-hover:scale-105">
                  {watchedData.personalInfo.profileImage ? (
                    <img src={watchedData.personalInfo.profileImage} alt="Profile" className="w-full h-full object-cover" />
                  ) : (
          ...
          {skillFields.map((field, index) => (
            <div key={field.id} className="p-3 border border-gray-100 rounded-lg bg-gray-50/50 flex flex-col gap-2 relative group">
              ...
              <input
                ...
                className="w-full px-2 py-1 bg-transparent border-b border-gray-200 outline-none text-sm"
              />
              <div className="flex items-center gap-3">
                <input
                  type="range"
